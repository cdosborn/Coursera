...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(dx,dy,exp) => case eval_prog(exp,env) of
                               ...
                               | _ => raise Impossible("exp cannot be shifted")

(* CHANGE: Add a case for Shift expressions *)

(* Pre:  Expects exp of dataype geom_exp
*  Post: Returns exp ensuring that line segments' are formatted where the first point
*        is always less in height and width. If not possible (points are eaual) a
*        point is returned.                                                          *)
   ...
   case gexp of
        LineSegment (x1,y1,x2,y2) => 
           ...
           then Point(x1,y1)
              ...
              val x1big = x1 > x2 + epsilon
              val y1big = y1 > y2 + epsilon
              val xs_eql = real_close(x1,x2)
                in if (xs_eql andalso y1big) orelse x1big
